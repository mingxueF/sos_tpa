...
folder_a = os.path.join(folder,"dis_a")
folder_s = os.path.join(folder,"dis_s")
...
coor_a = {}
coor_s = {}
M_a = np.zeros((shape,3,3)) 
M_s = np.zeros((shape,3,3))
...
#partial_Q = np.einsum('ji,jkl,ij->ikl',L,partial_C,L)
partial_Q = np.dot(np.transpose(L),partial_C)
partial_Q = np.dot(L,partial_Q)
...
for q in range(len(lamda)):
    if lamda[q] != 0: 
        sigma_v = tools.get_sigma(partial_Q[q,:,:])
        sigma_v /= (2*lamda[q])
