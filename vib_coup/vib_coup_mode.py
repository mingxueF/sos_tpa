...
folder = os.path.join(root, "vibration_coupling","gs_mode")#,"dis_bigger")
...
#L = np_eigen[1:,:]
lamda, L = tools.read_modes(folder) # vibration frequency in a.u.
tot_states = 10
considered_state = 2
displacement =  0.16
...
for q in range(len(lamda)):
    if lamda[q] != 0: 
        sigma_v = tools.get_sigma(partial_C[:,q].reshape(3,3))
        sigma_v /= (2*lamda[q])
