...
def read_tpa(path):
    ...
    tpa = {'Excited state':[],'Excitation energy':[],'Oscillator strength':[],'dipole moment':[],\
           'TPA cross section(sos)':[],'TPA cross section(dI)':[],"Transition dipole moment":[]}
    try:
        ...
        with open(out,'r') as f:
            ...
            for i,line in enumerate(rl):        
                if "Excited state" in line:
                     ...
                     tpa['TPA cross section(dI)'].append(round(ex_cro_dI,2))
...
def plot_contributions_entangled(pd_tpa, basis,T_e, considered_state, tot_states):
    ...
    for j in range(1,tot_states):
        ...
        plt.bar(j, sigma, width=0.5, color='tab:blue')
        plt.xticks(np.arange(1,11,1))
        plt.title("Final ETPA cross section for ES "+ str(considered_state) + ": " + str(round(sigma_sos,2)))
        plt.ylabel("TPA cross section contribution (a.u.)")
        plt.xlabel(r"Excited state (Three-state model)  $T_e (fs)$ = "+str(T_e/41))
        #plt.tight_layout()
